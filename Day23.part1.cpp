...
private:
    // Fixed board layout in a 13x5 grid
    // #############
    // #...........#
    // ###X#X#X#X###
    //   #X#X#X#X#
...
GameState GameState::moveIntoRoom(int x) const
{
    ...
    const int y = (board_[3][roomPos] == '.') ? 3 : 2;
...
std::vector<GameState> GameState::possibleMoves() const
{
    ...
    for (int room = 0; room < 4; ++room)
    {
        ...
        const char occupants[] = { board_[2][roomPos], board_[3][roomPos] };
        ...
        {
            auto newMoves = isAmphipod(occupants[0]) ? moveFromRoom(roomPos, 2) : moveFromRoom(roomPos, 3);
    ...
    for (int hallPos = 1; hallPos < 12; ++hallPos)
    {
        ...
        if (isAmphipod(type))
        {
            ...
            // Is the room either empty, or occupied by a single correct type?
            if (!isAmphipod(occupants[0]) &&
                (!isAmphipod(occupants[1]) || occupants[1] == type))
            {
               bool blocked = false;
