...
int main(int, char **)
{
    ...
    // Convert into 2D array, five times the size of the original map
    Array2D grid(5*data.size(), 5*data[0].size());
    for (int sy = 0; sy < 5; ++sy)
    {
        for (int sx = 0; sx < 5; ++sx)
        {
            ...
            for (int y = 0; y < data.size(); ++y)
            {
                ...
                {
                    int risk = (int(data[y][x] - '0') - 1 + offset) % 9 + 1;
                    grid.set(sx*data.size() + x, sy*data[0].size() + y, risk);
                }
            }
        }
    }
