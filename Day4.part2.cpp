...
int main(int, char **)
{
    ...
    // Setup a map of completed boards, so that we don't continue processing after
    // is has been completed
    std::unordered_map<int, int> completed;
    int completedCount{0};

    // For all numbers, play all boards until all completed
    for (const auto &num : numbers)
    {
        if (completedCount == boards.size())
            break;
        ...
        for (int i = 0; i < boards.size(); i++)
        {
            if (completed.find(i) == completed.end())
            {
                ...
                {
                    // If the board is complete, write to the output and store the score
                    std::cout << "BOARD " << i << " COMPLETE\n";
                    std::cout << "SCORE: " << boards[i].sumUnplayed() * num << "\n";
                    completed[i] = boards[i].sumUnplayed() * num;
                    ++completedCount;
                }
            }
        }
